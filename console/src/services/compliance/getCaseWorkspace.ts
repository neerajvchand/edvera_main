...
import type { RiskStage, CaseWorkflowStage, PacketStage, OutcomeStage } from "@/lib/caseStages";
...
function buildTierChecklist(
  ...
  return {
    ...
    tier2: [
      // Conference requirement is met by EITHER conference_held OR conference_attempted
      (() => {
        const held = extractItem(t2, "conference_held", "Conference held", "action");
        const attempted = extractItem(t2, "conference_attempted", "Conference attempted (good-faith effort)", "action");
        if (held.completed) return { ...held, label: "Conference held" };
        if (attempted.completed) return { ...attempted, key: "conference_held", label: "Conference attempted (good-faith effort)" };
        return { ...held, label: "Conference held or attempted" };
      })(),
...
function buildTimeline(
  ...
  for (const a of actions) {
    ...
    // Build effectiveness description for completed actions
    let desc: string;
    if (status === "completed") {
      const rateBefore = a.attendance_rate_before as number | null;
      const rateAfter = a.attendance_rate_after_30d as number | null;
      if (rateBefore != null && rateAfter != null) {
        const delta = Math.round((rateAfter - rateBefore) * 10) / 10;
        const arrow = delta > 0 ? "↑" : delta < 0 ? "↓" : "→";
        desc = `Completed · ${rateBefore}% ${arrow} ${rateAfter}% after 30d`;
      } else if (rateBefore != null) {
        desc = `Completed · Rate at completion: ${rateBefore}% (30d measurement pending)`;
      } else {
        desc = "Completed";
      }
    } else {
      desc = status === "deferred" ? "Deferred" : status;
    }
...
export async function getCaseWorkspace(
  ...
  const { data: caseRow, error: caseError } = await supabase
    ...
    .select(
      `id, student_id, school_id, academic_year, current_tier,
       ...
       risk_stage, case_workflow_stage, packet_stage, outcome_stage,
  ...
  const [
    ...
    supabase
      .from("actions")
      .select("id, action_type, title, description, status, created_at, completed_at, attendance_rate_before, attendance_rate_after_30d")
      .eq("compliance_case_id", caseId)
  ...
  const result: CaseWorkspaceResponse = {
    case: {
      ...
      riskStage: (caseRow.risk_stage as RiskStage) ?? "monitoring",
      caseWorkflowStage: (caseRow.case_workflow_stage as CaseWorkflowStage) ?? "new",
      packetStage: (caseRow.packet_stage as PacketStage) ?? "not_started",
      outcomeStage: (caseRow.outcome_stage as OutcomeStage) ?? null,
