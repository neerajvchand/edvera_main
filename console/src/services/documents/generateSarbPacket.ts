 ...
 *
 * Schema contract — fields fetched by this service:
 *   students: first_name, last_name, grade_level, birth_date
 *   enrollments: enter_date (earliest record for student+school)
 *   compliance_cases: all columns via *
 *   attendance_snapshots: days_enrolled, days_present, days_absent,
 *     days_absent_unexcused, attendance_rate, is_chronic_absent
 *   attendance_daily: attendance_date, status
 *   actions: id, action_type, title, status, due_date, completed_at,
 *     completion_data, completion_notes, created_at
 *   intervention_log: id, intervention_type, intervention_date,
 *     description, outcome, performed_by_name
 *   Last verified against canonical schema: 2026-03-16
 */
  ...
  try {
    ...
    const [{ data: caseData }, { data: actData }, { data: ivData }] = await Promise.all([
      supabase
        ...
        .select(
          `*, students!compliance_cases_student_id_fkey(first_name, last_name, grade_level, birth_date),
    ...
    // Fetch earliest enrollment date for this student+school
    const { data: enrollmentData } = await supabase
      .from("enrollments")
      .select("enter_date")
      .eq("student_id", caseData.student_id)
      .eq("school_id", caseData.school_id)
      .order("enter_date", { ascending: true })
      .limit(1)
      .maybeSingle();
    ...
    const packetData: SarbPacketData = {
      student: {
        ...
        dateOfBirth: (s?.birth_date as string) ?? null,
        enrollmentDate: enrollmentData?.enter_date ?? null,
      },
      school: {
