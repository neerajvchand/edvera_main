 ...
 *
 * Orchestrator that runs all four computation engines in sequence:
...
async function runActions(
  ...
  console.log(
    `[actions] fetched ${casesData.length} open cases, ${existingActionsData.length} existing actions`
  );
  ...
  console.log(`[actions] generated ${newActions.length} new actions`);
  ...
  const elapsed = (Date.now() - phaseStart) / 1000;
  console.log(
    `[actions] done: ${newActions.length - errorCount} inserted, ${errorCount} errors in ${elapsed.toFixed(1)}s`
  );

  return {
    new_actions: newActions.length - errorCount,
    by_type: byType,
    error_count: errorCount,
    elapsed_seconds: parseFloat(elapsed.toFixed(1)),
  };
