...
serve(async (req: Request) => {
  ...
  try {
    ...
    for (const snap of snapshots) {
      try {
        ...
        if (result.action === "create_case" && result.case_data) {
          // Phase F: Build initial timeline event
          const createEvent = {
            type: "case_created",
            tier: result.case_data.current_tier,
            reason: result.case_data.reason,
          ...
          inserts.push({
            ...
            // Four-layer state model: set initial stages
            risk_stage: "early_concern",
            case_workflow_stage: "new",
            // Phase F: Initial timeline
            timeline_events: JSON.stringify([createEvent]),
          });
          newCases++;
          ...
          // Four-layer state model: update risk_stage + case_workflow_stage on escalation
          const TIER_RISK_MAP: Record<string, string> = {
            tier_1_letter: "early_concern",
            tier_2_conference: "tier_2_support",
            tier_3_sarb_referral: "tier_3_intensive",
          };
          updateData.risk_stage = TIER_RISK_MAP[result.case_data.current_tier] ?? "early_concern";
          updateData.case_workflow_stage = "escalated";

          escalateOps.push({
            caseId: result.existing_case_id,
            data: updateData,
            reason: result.escalation_reason ?? `Escalated to ${result.case_data.current_tier}`,
          });

          // Mark prior tier actions as overdue on escalation
          const { error: overdueErr } = await supabase
            .from("actions")
            .update({ status: "overdue" })
            .eq("compliance_case_id", result.existing_case_id)
            .in("status", ["pending", "in_progress"]);
          if (overdueErr) {
            console.warn(
              `compute-compliance: failed to mark actions overdue for case ${result.existing_case_id}: ${overdueErr.message}`
            );
          }
    ...
    //    Phase F: Append timeline event for each escalation
    for (const op of escalateOps) {
      // Fetch existing timeline_events to append to
      ...
      const timeline: unknown[] = Array.isArray(existing?.timeline_events)
        ? existing.timeline_events
        : [];

      timeline.push({
        type: "tier_escalated",
        tier: op.data.current_tier,
        reason: op.reason,
        timestamp: new Date().toISOString(),
        auto: true,
      });

      op.data.timeline_events = JSON.stringify(timeline);
